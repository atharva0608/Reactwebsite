...
const Projects = ({ canvasDetails }) => {
  const projects = [
    ...
    {
      number: "03",
      title: " GitOps Deployment and Observability",
      description: "Prometheus | Grafana | ArgoCD",
      github: "Devops-Monitoring",
      image: "/images/project3.png"
    },
    {
      number: "02",
      title: "Production-Ready AKS Infrastructure with Observability, Service Mesh & API Gateway",
      description: "Azure AKS | Terraform | Helm | APISIX | Consul | Kubernetes  ",
      ...
      <div className="z-10 w-full md:w-[90%] lg:w-[80%] flex flex-col gap-8 mx-auto">
        {projects.map((project, index) => (
          <div key={index} className="relative flex flex-col items-center bg-[#36454F]/30 backdrop-blur-md rounded-lg border border-white/10 p-4 md:p-8">
            <a href={`https://github.com/atharva0608/${project.github}`} className="absolute top-4 right-4 z-20 transition-all duration-300 hover:scale-110">
            ...
            <div className={`flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} w-full items-center mt-16 md:mt-4`}>
              <div className={`hidden md:block text-6xl md:text-[120px] lg:text-[180px] font-bold text-white opacity-80 ${index % 2 === 0 ? 'mr-8' : 'ml-8'}`}>
                {project.number}
              </div>
              <div className={`flex flex-col max-w-[600px] text-center ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
                <h2 className="text-3xl md:text-4xl lg:text-4xl font-bold mb-4 pt-4">{project.title}</h2>
